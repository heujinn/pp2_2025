    ...
    for event in pygame.event.get():
        ...
        if pen=="square":
            ...
            if last_event=="not press":
                if (x1-x)<=(y1-y):
                    y1=y+(x1-x)#для равных граней квадрата
                else:
                    x1=x+(y1-y)#для равных граней квадрата
        ...
        if pen == "right triangle":
            ...
            if last_event=="not press":
                ...
                pygame.draw.line(screen, color, (x, y),(x, y1) , 1)#рисуем первую грань
                pygame.draw.line(screen, color, (x, y1),(x1, y1) , 1)
        ...
        if pen == "equilateral triangle":
            ...
            if last_event=="not press":
                h=y+((math.sqrt(3)*(x1-x))/2)#это высота. ориентируемся на высоту и половину нижнего основания
                ...
                #pygame.draw.line(screen, color, (x+(x1-x)/2, y), (x+(x1-x)/2, h), 1) #высота
                last_event = None  
        #rhombus
        if pen == "rhombus":
            if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                last_event = "not press"
            if last_event == "not press":
                pygame.draw.line(screen, color, (x, y+(y1-y)/2),(x+(x1-x)/2, y), 1)#для нахождения координат верхушки
